    ...
    def convert_data_to_id_pad_eos(self):
        ...
        with open(train_args.raw_train_english_after_tokenization_data_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                for token in line:
                    # 将token转换成id
                    token_id = english_token_id_dictionary.get(token, english_token_id_dictionary['<unk>'])
                    line_converted_to_id.append(str(token_id))
                # 在英文语料最后加上EOS
                line_converted_to_id.append(str(english_token_id_dictionary['<eos>']))
                # 写入本地文件
        ...
        with open(train_args.raw_train_chinese_after_tokenization_data_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                for token in line:
                    # 将token转换成id
                    token_id = chinese_token_id_dictionary.get(token, english_token_id_dictionary['<unk>'])
                    line_converted_to_id.append(str(token_id))
                # 因为这个中文语料是当做目标词的，因此也需要在中文语料最后面加上EOS
                # decoder的输入的最开始的BOS，会在train.py里面添加。
                line_converted_to_id.append(str(chinese_token_id_dictionary['<eos>']))
                # 写入本地文件
                fwrite.write(' '.join(line_converted_to_id)+'\n')
        fwrite.close()
