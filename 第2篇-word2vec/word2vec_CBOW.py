...
def batch_generation(batch_size, cbow_window):
    '''
    函数作用：
    产生batch_size个样本，cbow_window就是CBOW模型的滑动窗口中，中心词左边或者右边词数量。
    '''
    global data_index
...
with graph.as_default():
    ...
    train_dataset = tf.placeholder(tf.int32, shape=[batch_size,2 * cbow_window])  # 传进来的train_dataset是词的索引
    ...
    for i in range(2 * cbow_window):
        embedding_i = tf.nn.embedding_lookup(embeddings, train_dataset[:,i])  # 从词向量矩阵中，根据index找到对应的词的词向量。
