...
#参数配置=================================
# 语料库的下载地址
corpus_url = 'http://mattmahoney.net/dc/'
#参数配置=================================
...
def batch_generation(batch_size=8, num_skips=2, skip_window=1):
    ...
    函数作用：
    产生batch_size个样本
    num_skips就是一个窗口中，采几个样本。
    因此，batch_size有一定需要是num_skips的倍数，不然不能整除。
    默认情况下skip_window=1, num_skips=2。
    此时就是从连续的3(3 = skip_window*2 + 1)个词中生成2(num_skips)个样本。
    例子：假如连续的三个词['used', 'against', 'early']
    并且batch_size=8, num_skips=2, skip_window=1
    那么生成两个样本就是：
    against（输入） -> used（目标预测词）, against（输入） -> early（目标预测词）
    '''
    ...
    # [ skip_window target skip_window ] skip_window可以看成是窗口中心的词的两边词的数量。
    span = 2 * skip_window + 1
    # 是一个队列，FIFO的队列，其实就是滑动窗口。窗口大小为span
    ...
    for i in range(batch_size // num_skips):
        ...
        # 目标词在窗口的中心位置，由于默认skip_window为1
        # 因此窗口大小为3，因此位于窗口的skip_window索引位置的就是中心词。
...
valid_examples = np.random.choice(valid_window, valid_size,
                                  replace=False)  # 只对前valid_window的词进行验证

graph = tf.Graph()
with graph.as_default():
    # 定义输入和label的占位符
    train_inputs = tf.placeholder(tf.int32, shape=[batch_size])
    # 虽然skwip-gram模型要预测的是中心词两边的词
    # 但是实际上每次只放入一个词作为label，而不是中心词两边的所有的词。
    ...
    # tf.nn.nce_loss将矩阵相乘封装好了，直接传入接口需要的参数就可以了。
    # 随机选取num_sampled个噪声词
    # 由于并不需要即将预测的值的词向量，因此我们这里没有传入。
    # 只需要传入目标词的下标（train_labels），以及词典大小就可以了（vocabulary_size）。
    ...
    # 计算词和词的相似度，在这里，我们提前计算每个词向量的二范式
    # 因为余弦相似度的公式的分母中，是两个向量的二范式。
...
with tf.Session(graph=graph, config=config) as session:  # 打开session
    ...
    for step in range(num_steps):
        batch_inputs, batch_labels = batch_generation(batch_size, num_skips,
                                                      skip_window)
        ...
        if (step + 1) % 2000 == 0:
            ...
            print('Average loss at step {}-----loss is {}'.format(
                (step + 1), average_loss))
            total_loss = 0
